...
- **Q&A**는 `qna` 표에 담깁니다. 질문자의 이름·이메일과 비밀글이 들어가므로
  2단계의 RLS·권한 회수를 반드시 함께 실행하십시오.
- **지원서**(직원채용)는 `applications` 표에 담깁니다. 성함·이메일·연락처가
  들어가는 유일한 표이므로 2단계의 RLS·권한 회수를 반드시 함께 실행하십시오.
  읽기는 관리자 세션을 거친 서버 코드만 합니다.
