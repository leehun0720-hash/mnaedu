...
-- ─────────────────────────────────────────────────────────────
-- 0부. 안전장치 — 이 줄을 지우지 마십시오
-- ─────────────────────────────────────────────────────────────
--
-- CREATE/ALTER TABLE 은 표 전체를 독점해야 합니다. 누군가 그 표를 붙들고
-- 있으면(Table Editor 를 열어 두었거나, 다른 SQL 탭이 돌고 있으면) 이 문장은
-- 줄을 서고, 그 뒤로 들어오는 평범한 조회까지 함께 멈춥니다 — 관리자 화면이
-- "12초 안에 답하지 않았습니다"가 되는 것이 바로 그 상태입니다.
--
-- 아래 한 줄이 그것을 막습니다. 5초 안에 자리를 못 잡으면 조용히 줄 서는 대신
-- "canceling statement due to lock timeout" 이라고 분명히 실패합니다. 그러면
-- 표는 얼지 않고, 무엇이 문제인지도 그 자리에서 드러납니다.
SET lock_timeout = '5s';
