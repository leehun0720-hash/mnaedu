...
- **정본 분류 체계** — 「업무분야별 카테고리」의 **5분야 58주제**가 홈페이지 게시판과
  ...
  `lib/questions.ts`가 그대로 가져다 쓴다(문제은행 가로축). 표기는 그 위에
  **두 오피스 축**을 얹는다 — M&A 오피스(중개·분쟁·자금조달 3분야)와
  시크릿 오피스(패밀리오피스·투자가 클럽 2분야, `OFFICES`).
...
- **화면 테마 3종** — 엠버(기본)·감청·세피아. 색만 갈아 끼우는 방식이라
  대비를 좌우하는 토큰(`--ink`·`--ground` 등)은 건드리지 않고 액센트와 종이
  온도만 바꾼다. 선택은 브라우저에 저장되고, 첫 페인트 전 인라인 스크립트가
  적용해 깜빡임이 없다. 관리자 화면에는 두지 않는다(`app/theme-toggle.tsx`).
