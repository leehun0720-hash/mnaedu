...
- **공개 표현 순화** — 민감 주제명은 공개 목차에서 순화하고 원문은 `sourceLabel`에 남겨
  대조할 수 있게 한다.
- **복사 방지** — 우클릭·드래그·복사·인쇄를 억제한다(`app/copy-guard.tsx`). 관리자
  화면에는 적용하지 않는다. 완전 차단은 불가능하므로 핵심 자산은 회원 전용 열람으로 보완한다.
...
- **등급 수동 전환** — 결제 연결 전까지 유료 전환은 `/admin` 회원 관리에서 한다.
  SQL을 열 필요 없이 등급·포인트를 조정할 수 있고, 테스트 계정으로 무료/유료
  화면을 번갈아 확인하는 용도로도 쓴다.
...
- 사무실 상세 주소(동·층) 확인
- 유료 요금 확정 시 결제 연결 (금액 비노출 유지) — 그전까지 유료 전환은 관리자 수동
